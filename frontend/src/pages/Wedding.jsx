...
export default function Wedding() {
  ...
  const submit = async (e) => {
    e.preventDefault();
    try { await api.post('/enquiry/bulk', form); setSent(true); } catch { setSent(true); }
  };
  const list = hampers.filter((h) => h.tags.includes('Wedding') || h.tier === 'Luxury');

  return (
    <div>
      <section className="relative overflow-hidden bg-cream-300">
        <div className="absolute inset-0 opacity-30"><img src="https://loremflickr.com/1600/600/wedding,decoration,gift?lock=88" alt="" className="w-full h-full object-cover" /></div>
      ...
      <div className="sk-container py-14 md:py-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {[{Ic:Award,t:'Premium Quality',s:'Grade-A only'},{Ic:Package,t:'Custom Branding',s:'Your logo & cards'},{Ic:Users,t:'Bulk Pricing',s:'From 25 pieces'},{Ic:ShieldCheck,t:'Pan-India',s:'20,000+ pincodes'},{Ic:Heart,t:'On-Time Delivery',s:'Never miss the date'}].map(({Ic,t,s})=>(<div key={t} className="sk-card p-5 text-center"><Ic size={26} className="mx-auto text-brand-900" /><div className="font-display font-bold text-brand-900 mt-2">{t}</div><div className="text-[12px] text-ink-500 mt-0.5">{s}</div></div>))}
        </div>
      </div>
      ...
      <div className="sk-container py-14 md:py-20 grid md:grid-cols-2 gap-10" id="form">
        <div>
          ...
          <div className="mt-5 flex flex-wrap gap-3"><a href="https://wa.me/919876543210" target="_blank" rel="noreferrer" className="sk-btn-outline text-[var(--sk-green-500)] border-[var(--sk-green-500)]"><MessageCircle size={16} /> WhatsApp Us</a><a href="https://instagram.com" target="_blank" rel="noreferrer" className="sk-btn-outline"><Instagram size={16} /> Instagram DM</a></div>
        </div>
        <form onSubmit={submit} className="sk-card p-6 md:p-8 space-y-3">
          {sent ? (<div className="text-center py-8"><div className="font-display font-bold text-brand-900 text-2xl">Thank you!</div><p className="text-ink-600 mt-2">We’ll reach out shortly.</p></div>) : (<>
            <div className="grid md:grid-cols-2 gap-3"><input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="sk-input" placeholder="Full name" /><input required value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="sk-input" placeholder="Phone number" /></div>
            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="sk-input" placeholder="Email" />
            <select value={form.occasion} onChange={(e) => setForm({ ...form, occasion: e.target.value })} className="sk-input">{['Wedding','Reception','Sangeet','Engagement','Anniversary','Other'].map((o) => <option key={o}>{o}</option>)}</select>
