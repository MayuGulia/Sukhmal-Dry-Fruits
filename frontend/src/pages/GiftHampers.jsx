    ...
    <div>
      ...
      <div className="sk-container py-8 md:py-12">
        <div className="flex flex-wrap gap-2 mb-4">
          {CHIPS.map((c) => (
            <button key={c} onClick={() => setChip(c)} className={`px-4 py-2 rounded-full text-sm font-medium border transition ${chip === c ? 'bg-brand-900 text-white border-brand-900' : 'bg-white text-brand-900 border-line-strong hover:border-brand-900'}`}>{c}</button>
          ))}
        </div>
        <div className="flex flex-wrap gap-2 mb-8">
          {BUDGETS.map((b) => (
            <button key={b.l} onClick={() => setBudget(budget?.l === b.l ? null : b)} className={`px-4 py-1.5 rounded-full text-[12px] font-medium border transition ${budget?.l === b.l ? 'bg-gold-500 text-white border-gold-500' : 'bg-white text-brand-900 border-line-strong'}`}>{b.l}</button>
          ))}
        </div>

        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-5">
          {list.map((h) => <HamperCard key={h.id} h={h} />)}
        </div>
