...
const { json, readJsonBody, httpsFn, VERTEX_PREDICT_SA } = require('../_shared/httpFn');
    ...
    const message =
      code === 'bad_request' || code === 'no_match'
        ...
        : code === 'gemini_auth' || code === 'not_configured'
          ? (err.message || 'Vertex AI predict denied. In Google Cloud, open project sukhmal-website → IAM & Admin → IAM (not Service Accounts) → Grant access to 1035357939609-compute@developer.gserviceaccount.com and firebase-adminsdk-fbsvc@sukhmal-website.iam.gserviceaccount.com with role Vertex AI User, and enable the Vertex AI API.')
          : (err.message || CUSTOMER_AI_FALLBACK);
    return json(res, { error: code, message }, status);
  }
}, { timeoutSeconds: 60, serviceAccount: VERTEX_PREDICT_SA });
