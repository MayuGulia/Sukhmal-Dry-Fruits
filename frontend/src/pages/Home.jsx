...
import { PRODUCTS, HAMPERS, CATEGORIES, TESTIMONIALS, INSTAGRAM_POSTS, FESTIVALS } from '@/data/mockCatalog';
    ...
    <div>
      ...
      <section className="sk-container py-14 md:py-20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
          {CATEGORIES.map((c) => (
            <Link key={c.slug} to={c.slug === 'gift-hampers' ? '/gift-hampers' : `/category/${c.slug}`} className="sk-card block group" data-testid={`cat-${c.slug}`}>
              ...
              <div className="p-3 text-center">
                ...
                <div className="text-[11px] text-ink-500 mt-0.5 line-clamp-2">{c.tagline}</div>
              </div>
            </Link>
          ))}
        </div>
            ...
            {FESTIVALS.map((f) => (
              <Link key={f.key} to={`/festival-collections#${f.key}`} className="sk-card block w-[240px] md:w-auto" data-testid={`fest-${f.key}`}>
                <div className="aspect-[4/3] overflow-hidden bg-cream-300" style={{ background: `linear-gradient(135deg, ${f.hue}22, ${f.hue}66)` }}>
                  <img src={f.hero} alt={f.name} className="w-full h-full object-cover mix-blend-multiply" loading="lazy" />
                </div>
      ...
      <section className="bg-cream-300">
        <div className="sk-container py-16 md:py-24 grid md:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative rounded-2xl overflow-hidden shadow-sk-lg">
            <img src={verifiedImg('build-hamper-promo', 1200)} alt="Custom hamper" className="w-full h-[420px] object-cover" />
          </div>
        </div>
      </section>
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div className="sk-card p-6 md:p-8">
            ...
            <div className="mt-4 flex gap-2">
              <input placeholder="e.g. Diwali hamper with almonds, cashews & dates in a wooden basket" className="sk-input" />
              <button className="sk-btn-gold whitespace-nowrap"><Sparkles size={16} /> Generate</button>
            </div>
      ...
      <section className="bg-cream-200 py-16 md:py-24">
        <div className="sk-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[
              { Ic: Award, t: '30+ Years', s: 'of Trust & Experience' },
              { Ic: Leaf, t: '100% Natural', s: 'No preservatives' },
              { Ic: Star, t: 'Premium Quality', s: 'Grade-A only' },
              { Ic: Package, t: 'Hygienic Packing', s: 'Vacuum-sealed' },
              { Ic: Truck, t: 'Fast Delivery', s: 'Pan-India express' },
              { Ic: Users, t: '50,000+ Customers', s: 'Loved us since 1994' },
            ].map(({ Ic, t, s }) => (
              <div key={t} className="sk-card p-5 text-center">
                <div className="h-12 w-12 mx-auto rounded-full bg-cream-300 grid place-items-center text-brand-900 mb-3"><Ic size={22} /></div>
                <div className="font-display font-bold text-brand-900">{t}</div>
      ...
      <section className="sk-container py-16 md:py-24 grid md:grid-cols-2 gap-6">
        {[
          { to: '/wedding-gifts', title: 'Wedding Gifts', copy: 'Elegantly crafted hampers for the wedding season — bulk pricing and custom branding available.', img: 'https://images.unsplash.com/photo-1519741497674-611481863552?w=1200&auto=format&fit=crop' },
          { to: '/corporate-gifts', title: 'Corporate Gifting', copy: 'Premium hampers with your logo, personalised cards, and GST invoicing — delivered pan-India.', img: 'https://images.unsplash.com/photo-1502920917128-1aa500764cbd?w=1200&auto=format&fit=crop' },
        ].map((b) => (
