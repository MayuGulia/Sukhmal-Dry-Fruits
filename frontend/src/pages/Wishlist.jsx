  ...
  return (
    <div>
      ...
      <div className="sk-container py-8">
        {loading ? (<div className="grid grid-cols-2 md:grid-cols-4 gap-4">{Array.from({length:4}).map((_,i)=><ProductSkeleton key={i} />)}</div>) : items.length === 0 ? (
          ...
          <>
            <div className="flex justify-end mb-4 gap-2"><button onClick={() => { items.forEach((p) => add(p, { qty: 1, variant: p.variants?.[0] })); }} className="sk-btn-primary text-sm"><ShoppingBag size={14} /> Move All to Cart</button><button onClick={clear} className="sk-btn-outline text-sm"><Trash2 size={14} /> Clear</button></div>
              ...
              {items.map((p) => (
                <div key={p.id} className="sk-card overflow-hidden">
                  ...
                  <div className="p-3">
                    ...
                    <div className="mt-2 grid grid-cols-2 gap-2"><button onClick={() => { add(p, { qty: 1, variant: p.variants?.[0] }); toggle(p.id); }} className="sk-btn-primary text-[12px] !py-1.5"><ShoppingBag size={12} /> Cart</button><button onClick={() => toggle(p.id)} className="sk-btn-outline text-[12px] !py-1.5"><Trash2 size={12} /> Remove</button></div>
                  </div>
                </div>
              ))}
            </div>
